...
const Heading: React.FunctionComponent<Props & React.HTMLAttributes<Props>> = ({
  level,
  asLevel,
  className,
  ...props
  ...
  const headingLevel =
    level || Math.min(React.useContext(HeadingLevelContext), 6);
  ...
  return (
    <Elem
      {...props}
      className={cx(styles[`h${asLevel || headingLevel}`], className)}
    />
  );
};
